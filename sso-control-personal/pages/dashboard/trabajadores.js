      ...
      <form onSubmit={handleCreate} className="bg-white rounded-card shadow-card p-5 flex flex-wrap gap-3 items-end mb-6">
        <div className="w-full sm:w-auto">
          ...
          <input
            ...
            className="border rounded-card px-3 py-2 text-sm w-full sm:min-w-[14rem]"
            placeholder="Ej. Juan Pérez"
          />
        </div>
        <div className="w-full sm:w-auto">
          <label className="block text-xs font-medium text-ink/60 mb-1">Puesto</label>
          <select value={position} onChange={(e) => setPosition(e.target.value)} className="border rounded-card px-3 py-2 text-sm w-full">
        ...
        <div className="w-full sm:w-auto">
          ...
          <select value={companyId} onChange={(e) => setCompanyId(e.target.value)} className="border rounded-card px-3 py-2 text-sm w-full">
            {companies.map((c) => (
